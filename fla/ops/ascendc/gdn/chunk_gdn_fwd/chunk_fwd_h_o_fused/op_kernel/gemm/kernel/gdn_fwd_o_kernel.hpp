...
public:
    ...
    __aicore__ inline bool CanRunChunkPipeline() const
    {
        ...
        return isVariedLen == 0 &&
               AscendC::GetBlockNum() == 2 * shapeBatch * vNumHead;
    }
    ...
    __aicore__ inline void WaitProducerSliceReady(const GDNFwdOOffsets &offsets)
    {
        ...
        const uint32_t producerAivIdx = producerPairIdx * AscendC::GetSubBlockNum() +
    ...
    __aicore__ inline void Init(GM_ADDR q, GM_ADDR k, GM_ADDR v, GM_ADDR h, GM_ADDR g,
        GM_ADDR cu_seqlens, GM_ADDR chunk_offsets, GM_ADDR o, const GDN::ChunkFwdHOFusedOStageTilingData *tilingData, GM_ADDR user) {
        ...
        if ASCEND_IS_AIC {
            ...
            while (cubeBlockScheduler.isRunning) {
                ...
                if (needRun && coreIdx < coreNum) {
                    ...
                    // matching IBWait has completed. The AIC wait aggregates
                    // those two acknowledgements, so the complete H/V tiles
                    // are visible before their first Cube-side GM read.
        ...
        if ASCEND_IS_AIV {
            ...
            while (vecBlockScheduler.isRunning) {
                ...
                if (vecBlockScheduler.isRunning && coreIdx < coreNum * subBlockNum) {
                    ...
                    WaitProducerSliceReady(vec1Offsets);
                    // Reuse cube1Done in the reverse AIV-to-AIC direction.
                    // The AIC has already published and both AIVs have consumed
                    // the QK-ready generation before this acknowledgement.
