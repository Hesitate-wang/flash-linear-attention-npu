...
struct BlockSchedulerGdnFwdO {
    ...
    void Init(GM_ADDR cu_seqlens, GM_ADDR chunk_offsets, const GDN::ChunkFwdHOFusedOStageTilingData *tilingData,
        ...
        if (chunkPipeline) {
            const uint32_t producerCoreNum = shapeBatch * vNumHead;
            const uint32_t consumerCoreBegin = producerCoreNum;
            const uint32_t consumerCoreEnd = consumerCoreBegin +
                                             producerCoreNum * GDN_FWD_HO_CONSUMERS_PER_PAIR;
            if (cubeCoreIdx < consumerCoreBegin || cubeCoreIdx >= consumerCoreEnd) {
                taskIdx = taskNum;
                isRunning = false;
            } else {
                const uint32_t consumerIdx = cubeCoreIdx - consumerCoreBegin;
                const uint32_t pipelineBatchIdx = consumerIdx / vNumHead;
                pipelineHeadIdx = consumerIdx % vNumHead;
                pipelineLaneIdx = 0;
                workspaceCoreIdx = consumerIdx;
                taskIdx = pipelineBatchIdx * numChunks * vNumHead + pipelineHeadIdx;
    ...
    void InitTask() {
        ...
        if (chunkPipeline) {
            ...
            taskIdx += GDN_FWD_HO_CONSUMERS_PER_PAIR * vNumHead;
        } else if (taskAffinity) {
